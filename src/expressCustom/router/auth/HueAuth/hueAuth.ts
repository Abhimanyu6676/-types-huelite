...
hueAuthRouter.post('/signin', async (req: any, res: any) => {
    ...
    authenticateWithEmail(req, res, log).then(({ success }) => {
        log("fetching userData")
        DB_getUser_email({ email: req?.body?.email }, log).then((user) => {
            if (user) {
...
const authenticateWithEmail = async (req: any, res: any, _log?: logFun_t): Promise<{ success: boolean }> => new Promise(async (resolve, reject) => {
